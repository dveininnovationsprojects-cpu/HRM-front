    ...
    return (
        <DashboardLayout role="MANAGER" title="Project Architecture">
            <div style={{ fontFamily: "'Inter', sans-serif", paddingBottom: '40px' }}>
                ...
                {/* ========================================== */}
                <div style={{ background: colors.cardWhite, padding: '32px', borderRadius: '20px', border: `1px solid ${colors.border}`, marginBottom: '32px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.02)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                        <div style={{ background: colors.lightBlue, padding: '12px', borderRadius: '12px', color: colors.primaryBlue }}>
                            <UploadCloud size={24} />
                        </div>
                        <h3 style={{ margin: 0, fontSize: '20px', color: colors.mainText, fontWeight: '700' }}>Initialize New Architecture</h3>
                    </div>
                    ...
                    <form onSubmit={handleProjectUpload} style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '24px' }}>
                        ...
                        {/* TL Selection */}
                        <div>
                            <label style={{ display: 'block', fontSize: '14px', fontWeight: '600', color: colors.mainText, marginBottom: '8px' }}>Assign Lead Architect (TL)</label>
                            <select 
                                required 
                                value={tlBiometricId} 
                                onChange={(e) => setTlBiometricId(e.target.value)}
                                style={{ width: '100%', padding: '14px', borderRadius: '12px', border: `1px solid ${colors.border}`, outline: 'none', background: colors.inputBg, fontSize: '15px', color: colors.mainText, cursor: 'pointer' }}
                            >
                            ...
                            {teamLeads.length === employees.length && employees.length > 0 && (
                                <p style={{ margin: '6px 0 0', fontSize: '12px', color: colors.warningText }}>* Showing all employees as no specific TLs were found.</p>
                            )}
                        </div>

                        {/* Drag & Drop Zone */}
                        <div>
                            <label style={{ display: 'block', fontSize: '14px', fontWeight: '600', color: colors.mainText, marginBottom: '8px' }}>Project Requirement File (Excel/CSV)</label>
                            <div 
                            ...
                            >
                                ...
                                <h4 style={{ margin: '0 0 8px 0', color: colors.mainText, fontSize: '16px' }}>
                                    {file ? file.name : "Drag & Drop your Excel file here"}
                                </h4>
                                <p style={{ margin: 0, color: colors.secondaryText, fontSize: '13px' }}>
                ...
                {/* ========================================== */}
                <div style={{ background: colors.cardWhite, padding: '24px', borderRadius: '20px', border: `1px solid ${colors.border}`, boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                    
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
                        <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '10px', color: colors.mainText, fontSize: '20px', fontWeight: '700' }}>
                            <Briefcase size={22} color={colors.primaryBlue} /> Active Projects Inventory
                        </h3>
                        ...
                        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                            <div style={{ position: 'relative' }}>
                                ...
                                <input 
                                    ...
                                    style={{ padding: '10px 10px 10px 36px', borderRadius: '8px', border: `1px solid ${colors.border}`, background: colors.inputBg, outline: 'none', fontSize: '13px', width: '200px' }}
                                />
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', background: colors.inputBg, padding: '2px 10px', borderRadius: '8px', border: `1px solid ${colors.border}` }}>
                                <Filter size={14} color={colors.secondaryText} style={{ marginRight: '6px' }} />
                                <select 
                                    value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
                                    style={{ border: 'none', background: 'transparent', outline: 'none', fontSize: '13px', fontWeight: '600', color: colors.mainText, cursor: 'pointer' }}
                                >
                                ...
                                {loading ? (
                                    <tr>
                                        <td colSpan="5" style={{ padding: '60px', textAlign: 'center' }}>
                                            ...
                                            <p style={{ color: colors.secondaryText, fontWeight: '500' }}>Syncing architecture from database...</p>
                                        </td>
                                    </tr>
                                        ...
                                        >
                                            <td style={{ padding: '16px', borderRadius: '12px 0 0 12px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    ...
                                                    <div>
                                                        <h4 style={{ margin: '0 0 4px 0', color: colors.mainText, fontSize: '15px', fontWeight: '700' }}>{proj.projectName}</h4>
                                                    </div>
                                                </div>
                                            </td>
                                            <td style={{ padding: '16px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: colors.secondaryText, fontSize: '13px', marginBottom: '4px' }}>
                                                    <Calendar size={14} /> Deadline: <strong style={{ color: colors.mainText }}>{proj.deadline || 'Not Set'}</strong>
                                                </div>
                                            </td>
                                            <td style={{ padding: '16px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: colors.mainText, fontSize: '14px', fontWeight: '700' }}>
                                            ...
                                            <td style={{ padding: '16px', textAlign: 'right', borderRadius: '0 12px 12px 0' }}>
                                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                                                    <button 
                                                        ...
                                                        style={{ background: '#fff', color: colors.primaryBlue, border: `1px solid ${colors.border}`, padding: '8px', borderRadius: '8px', cursor: 'pointer' }} title="View Details"
                                                    >
                                                        <Eye size={16} />
                                                    </button>
                                                    <button 
                                                        onClick={(e) => handleDeleteProject(proj.id, e)}
                                                        style={{ background: colors.dangerBg, color: colors.dangerText, border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer' }} title="Revoke Project"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                    ...
                                    <tr>
                                        <td colSpan="5" style={{ padding: '60px', textAlign: 'center', color: colors.secondaryText, background: colors.inputBg, borderRadius: '12px' }}>
                                            <ShieldAlert size={48} style={{ opacity: 0.3, margin: '0 auto 16px' }} />
                                            <p style={{ margin: 0, fontSize: '15px', fontWeight: '500' }}>No projects match your criteria.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                            ...
                            <div style={{ padding: '24px' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
                                    <div style={{ background: colors.inputBg, padding: '16px', borderRadius: '12px' }}>
                                        <p style={{ margin: '0 0 4px 0', fontSize: '12px', color: colors.secondaryText, fontWeight: '600', textTransform: 'uppercase' }}>Current Status</p>
                                        <p style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: selectedProject.status === 'COMPLETED' ? colors.successText : colors.warningText }}>{selectedProject.status || 'ACTIVE'}</p>
                                    </div>
                                    <div style={{ background: colors.inputBg, padding: '16px', borderRadius: '12px' }}>
                                        <p style={{ margin: '0 0 4px 0', fontSize: '12px', color: colors.secondaryText, fontWeight: '600', textTransform: 'uppercase' }}>Total Workload</p>
                                        <p style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: colors.mainText }}>{selectedProject.totalEstimatedHours || 0} Hours</p>
                                    </div>
                                </div>

                                <div style={{ borderTop: `1px solid ${colors.border}`, paddingTop: '20px' }}>
                                    <p style={{ margin: '0 0 12px 0', fontSize: '14px', fontWeight: '700', color: colors.mainText }}>Project Meta Data</p>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: `1px dashed ${colors.border}` }}>
                                        <span style={{ color: colors.secondaryText, fontSize: '14px' }}>Deadline Limit</span>
                                        <strong style={{ color: colors.mainText, fontSize: '14px' }}>{selectedProject.deadline || 'Not Configured'}</strong>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: `1px dashed ${colors.border}` }}>
                                        <span style={{ color: colors.secondaryText, fontSize: '14px' }}>Daily Required Rate</span>
                                        <strong style={{ color: colors.mainText, fontSize: '14px' }}>{selectedProject.dailyRequiredHours || 0} Hrs/Day</strong>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0' }}>
                                        <span style={{ color: colors.secondaryText, fontSize: '14px' }}>Duration Frame</span>
                                        <strong style={{ color: colors.mainText, fontSize: '14px' }}>{selectedProject.durationDays || 30} Days</strong>
                                    </div>
                                </div>
                            </div>

                            <div style={{ padding: '16px 24px', background: '#F8FAFC', borderTop: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'flex-end' }}>
                                <button onClick={() => setIsModalOpen(false)} style={{ background: colors.primaryBlue, color: '#fff', border: 'none', padding: '10px 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
                                    Close Inspector
                                </button>
                            </div>
                        </div>
